...
| --- | --- | --- |
| 1 | Extraction | Content script uses MutationObserver to detect posts as they load in the feed. Multiple CSS selector fallbacks handle LinkedIn DOM changes. |
...
LinkedIn frequently changes their DOM structure. The extension uses multiple fallback selectors for each element type (posts, text, authors, buttons). If classification stops working after a LinkedIn update:

1. Inspect the feed DOM in DevTools
2. Update the `SELECTORS` object in `content.js`
...
- LinkedIn's DOM structure may change, requiring selector updates
